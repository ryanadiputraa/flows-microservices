...
func (m *AuthMiddleware) ClaimJWTToken(next http.HandlerFunc) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			response.WriteErrorResponse(w, r, http.StatusUnauthorized, "missing token header", response.UNAUTHENTICATED, nil)
			return
		}

		claims, err := m.jwt.ParseJWTClaims(r.Context(), token)
		if err != nil {
			response.WriteErrorResponse(w, r, http.StatusForbidden, "invalid jwt token", response.FORBIDDEN, nil)
			return
		}
